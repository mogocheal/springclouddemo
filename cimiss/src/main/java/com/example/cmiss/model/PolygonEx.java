 ...
 * @since       1.0
 */
public class PolygonEx implements Shape, java.io.Serializable {
     ...
     * @since 1.0
     */
    public PolygonEx(float xpoints[], float ypoints[], int npoints) {
    ...
    public void translate(float dx, float dy) {
        ...
        if (dx < 0) {
            ...
            if (newv > oldv) {
                ...
                if (bounds.width >= 0) {
                	...
                	bounds.width += newv - Float.MIN_VALUE;
            ...
            if (newv < oldv) {
                ...
                if (bounds.width >= 0) {
                	...
                	bounds.width += newv - Float.MAX_VALUE;
                    // With large widths and large displacements
                    // we may overflow so we need to check it.
                    if (bounds.width < 0) bounds.width = Float.MAX_VALUE;
                }
        ...
        if (dy < 0) {
            ...
            if (newv > oldv) {
                // negative overflow
                if (bounds.height >= 0) {
                	bounds.height += newv - Float.MIN_VALUE;
            ...
            if (newv < oldv) {
                // positive overflow
                if (bounds.height >= 0) {
                	bounds.height += newv - Float.MAX_VALUE;
                    if (bounds.height < 0) bounds.height = Float.MAX_VALUE;
                }
                newv = Integer.MAX_VALUE;
            }
        }
	...
	public PolygonPathIterator(PolygonEx pg, AffineTransform at) {
	    poly = pg;
	    transform = at;
	    if (pg.npoints == 0) {
